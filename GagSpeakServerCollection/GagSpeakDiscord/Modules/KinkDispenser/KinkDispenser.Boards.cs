...
public partial class KinkDispenser : InteractionModuleBase
{
    ...
    public async Task ViewBoardPage()
    {
        ...
        if (_botServices.BoardData.TryGetValue(msgId, out var service))
        {
            ...
            string path = Path.Combine(basePath, "Downloads-Boards", $"board-thumbnail.{extension}");
    ...
    public async Task FinishDisplayWheelBoard()
    {
        ...
        if (_botServices.BoardData.TryGetValue(msgId, out var service))
        {
            ...
            string basePath = "/root/GagSpeak-WebService/GagSpeakServerCollection/GagSpeakDiscord/DownloadsFolder/";
            string picPath = Path.Combine(basePath, "Downloads-Boards", $"{msgId}-boardpage-highres.{extension}");
