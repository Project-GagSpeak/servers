...
using GagSpeak.API.Data.Extensions;
...
public partial class GagSpeakHub
{
    ...
    public async Task<List<OnlineUserIdentDto>> UserGetOnlinePairs(CensusDataDto? censusData)
    {
        ...
        _GagSpeakCensus.PublishStatistics(UserUID, censusData);
    ...
    {
        _logger.LogCallInfo(GagSpeakHubLogger.Args(dto.CharaData.FileReplacements.Count));
        ...
        foreach (var replacement in dto.CharaData.FileReplacements.SelectMany(p => p.Value))
        {
            ...
            {
                _logger.LogCallWarning(GagSpeakHubLogger.Args("Invalid Data", "GamePaths", validGamePaths, string.Join(",", invalidPaths), "Hash", validHash, replacement.Hash, "FileSwap", validFileSwapPath, replacement.FileSwapPath));
                hadInvalidData = true;
        ...
        _GagSpeakCensus.PublishStatistics(UserUID, dto.CensusDataDto);

        _GagSpeakMetrics.IncCounter(MetricsAPI.CounterUserPushData);
        _GagSpeakMetrics.IncCounter(MetricsAPI.CounterUserPushDataTo, recipientUids.Count);
    }
    ...
    public async Task UserReportProfile(UserProfileReportDto dto)
    {
        ...
        await Clients.User(dto.User.UID).Client_ReceiveServerMessage(MessageSeverity.Warning, "Your GagSpeak profile has been reported and disabled for admin validation").ConfigureAwait(false);
