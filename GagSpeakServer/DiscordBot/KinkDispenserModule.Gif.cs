...
public partial class KinkDispenserModule : InteractionModuleBase
{
    ...
    public KinkDispenserModule(ILogger<GagspeakModule> logger, IServiceProvider services,
        IConfigService<DiscordConfiguration> gagspeakDiscordConfiguration, DiscordBotServices botServices,
    ...
    public async Task FinishDisplayWheel()
    {
        ...
        if(_botServices.GifData.TryGetValue(msgId, out var service)) 
        {
            ...
            string gifPath = Path.Combine("Downloads-Gifs", $"{msgId}-fullres-display.gif");
