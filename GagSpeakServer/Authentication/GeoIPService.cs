using GagspeakSynchronosShared;
using GagspeakSynchronosShared.Services;
using GagspeakSynchronosShared.Utils.Configuration;
...
public class GeoIPService : IHostedService
{
    ...
    private readonly IConfigurationService<AuthServiceConfiguration> _gagspeakConfiguration;
    ...
    public GeoIPService(ILogger<GeoIPService> logger,
        IConfigurationService<AuthServiceConfiguration> gagspeakConfiguration)
    {
        _logger = logger;
        _gagspeakConfiguration = gagspeakConfiguration;
    }
    ...
    private async Task PeriodicReloadTask(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            try
            {
                ...
                var useGeoIP = _gagspeakConfiguration.GetValueOrDefault(nameof(AuthServiceConfiguration.UseGeoIP), false);
                var cityFile = _gagspeakConfiguration.GetValueOrDefault(nameof(AuthServiceConfiguration.GeoIPDbCityFile), string.Empty);
